...
! versions of linux  ! SYS_SIZE是要加载的节数（16字节为1节）。0x3000共为0x3000字节=192kB，对于当前的版本空间已足够了
!
SYSSIZE = 0x3000
!
...
ROOT_DEV = 0x306  ! 指定根文件系统设备是第 2 个硬盘的第 1 个分区。这是 Linux 老式的硬盘命名
! 方式,具体值的含义如下:
...
ok2_read:
	...
	jne ok3_read	! 读该磁道的下一磁头面(1 号磁头)上的数据。如果已经完成,则去读下一磁道。
	mov ax,#1
	sub ax,head	! 判断当前磁头号。
